...
export const registerUser = async (req, res) => {
    ...
    try {
        ...
        const userId = await createUser(username, hashedPw, nickname, email);
...
export const loginUser = async (req, res) => {
  ...
  try {
    ...
    if (!user) return res.status(401).json({success: false, message: '등록된 사용자가 아닙니다.' });
    ...
    return res.json({success: true, message: '로그인 성공', token, username: user.username, userId: user.id });
  } catch (err) {
    console.error(err);
