...
import {
  bringReviewsByMovieId,
  bringReviewsByUserId,
  getMostReviewedMoviesWithCommentCount,
  rewriteReview,
  createReview,
  deleteReview,
...
//가장 리뷰가 많은 영화 중 댓글이 많은 리뷰 하나를 가져오기 - api/reviews/highlight
export const getMostReviewedMoviesWithTopComment = async (req, res) => {
  try {
    const result = await getMostReviewedMoviesWithCommentCount();
    res.json(result);
  } catch (error) {
    console.error('getMostReviewedWithTopComment 오류', error);
    res.status.json({ message: '서버 오류' });
  }
};
...
export const updateReview = async (req, res) => {
  ...
  try {
    ...
    if (result === 0) {
      return res
        .status(403)
        .json({ message: '수정 권한이 없습니다. 본인만 수정할 수 있습니다.' });
...
export const removeReview = async (req, res) => {
  ...
  try {
    ...
    if (result === 0) {
      return res
        .status(403)
        .json({ message: '삭제 권한이 없습니다. 본인만 삭제할 수 있습니다.' });
      //http 응답코드 403(Forbidden)
    }
